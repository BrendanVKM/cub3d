...
/*   Updated: 2025/04/14 14:03:30 by lemarian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	tex_stretch(t_data *data, t_raycast *rc, double line_height)
{
	...
	tex_pos = (line_start - WIN_HEIGHT / 2 + line_height / 2) * step; // is it WIN_HEIGHT or tile height?
	while (y < line_end)
	{
		tex_y = (int)tex_pos & (tex_height - 1);
		tex_pos += step;
		color = texture[tex_height * tex_y + tex_y];
		if (side == 1)
			color = (color >> 1) & 83355711;
		buffer[y][x] = color; // need to create and store buffer somewhere
		y++;
	}
}

void	texture_rendering(t_data *data, t_raycast *rc, double ray_dist, int side)
{
	...
	if (side == 0 && rc->ray_dir.x > 0) // texture is handled differently depending on direction
