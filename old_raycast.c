...
/*   Updated: 2025/04/22 14:28:12 by lemarian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	dda(t_data *data, t_raycast *rc)
{
	...
	while (1)
	{
		...
		if (data->config->map[rc->map_x][rc->map_y] == '1')//check if right x y order
			break;
	}
	return (get_wall_dist(rc));
}

void	raycast(t_data *data)//init rc/player_dir first
{
	int	x;
	double	cam_x;
	double	ray_dist;
